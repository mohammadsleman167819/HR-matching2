...
# Base form for Employee to reduce repetition
class EmployeeBaseForm(forms.ModelForm):
    firstname = forms.CharField(
        label="First Name", max_length=100, 
        help_text='Employee First Name', widget=forms.TextInput(attrs={'class': 'form-control'})
    )
    lastname = forms.CharField(
        label="Last Name", max_length=100, 
        help_text='Employee Last Name', widget=forms.TextInput(attrs={'class': 'form-control'})
    )
    dateOfBirth = forms.DateField(
        label="Date Of Birth", help_text='Employee Date of Birth', 
        widget=forms.DateInput(format="%Y-%m-%d", attrs={"type": "date", 'class': 'form-control'})
    )
    gender = forms.ChoiceField(
        choices=[('MALE', 'MALE'), ('FEMALE', 'FEMALE')], 
        label="Gender", widget=forms.Select(attrs={'class': 'form-control'})
    )
    city = forms.CharField(
        label="City", max_length=50, help_text='Employee City', 
        widget=forms.TextInput(attrs={'class': 'form-control'})
    )
    phone = forms.CharField(
        label="Phone", max_length=20, min_length=10, 
        help_text='Employee Phone', widget=forms.TextInput(attrs={'class': 'form-control'})
    )
    education = forms.CharField(
        label="Education", max_length=1000, required=False, 
        widget=forms.Textarea(attrs={'class': 'form-control'})
    )
    experience = forms.CharField(
        label="Experience", max_length=1000, required=False, 
        widget=forms.Textarea(attrs={'class': 'form-control'})
    )
    awards = forms.CharField(
        label="Awards", max_length=1000, required=False, 
        widget=forms.Textarea(attrs={'class': 'form-control'})
    )
    hobbies = forms.CharField(
        label="Hobbies", max_length=1000, required=False, 
        widget=forms.Textarea(attrs={'class': 'form-control'})
    )
    skills = forms.CharField(
        label="Skills", max_length=1000, 
        widget=forms.Textarea(attrs={'class': 'form-control'})
    )
    references = forms.CharField(
        label="References", max_length=1000, required=False, 
        widget=forms.Textarea(attrs={'class': 'form-control'})
    )
    other = forms.CharField(
        label="Other", max_length=1000, required=False, 
        widget=forms.Textarea(attrs={'class': 'form-control'})
    )
...
class EmployeeSignUpForm(CustomUserCreationForm, EmployeeBaseForm):
    class Meta(CustomUserCreationForm.Meta, EmployeeBaseForm.Meta):
        model = User
        fields = CustomUserCreationForm.Meta.fields + EmployeeBaseForm.Meta.fields
